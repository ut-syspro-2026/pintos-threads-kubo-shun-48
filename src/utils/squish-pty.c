...
static void make_noncanon(int fd, int vmin, int vtime) {
  if (isatty(fd)) {
    ...
    if (tcgetattr(fd, &termios) < 0) fail_io("tcgetattr");
    termios.c_lflag &= ~(ICANON | ECHO);
    termios.c_cc[VMIN] = vmin;
    termios.c_cc[VTIME] = vtime;
    if (tcsetattr(fd, TCSANOW, &termios) < 0) fail_io("tcsetattr");
  }
}
...
static void make_nonblocking(int fd, bool nonblocking) {
  ...
  if (flags < 0) fail_io("fcntl");
  if (nonblocking)
    flags |= O_NONBLOCK;
  else
    flags &= ~O_NONBLOCK;
  if (fcntl(fd, F_SETFL, flags) < 0) fail_io("fcntl");
}
...
static void relay(int pty, int dead_child_fd) {
  ...
  while (pipes[1].in != -1) {
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (i == 0 && !pipes[1].active) continue;

      if (p->in != -1 && p->size + p->ofs < sizeof p->buf)
        FD_SET(p->in, &read_fds);
      if (p->out != -1 && p->size > 0) FD_SET(p->out, &write_fds);
    }
    ...
    if (retval < 0) fail_io("select");

    if (FD_ISSET(dead_child_fd, &read_fds)) break;
...
static void sigchld_handler(int signo __attribute__((unused))) {
  if (write(dead_child_fd, "", 1) < 0) _exit(1);
}
...
int main(int argc __attribute__((unused)), char *argv[]) {
  ...
  if (master < 0) fail_io("open \"/dev/ptmx\"");
  if (grantpt(master) < 0) fail_io("grantpt");
  if (unlockpt(master) < 0) fail_io("unlockpt");

  /* Open slave side of pty. */
  name = ptsname(master);
  if (name == NULL) fail_io("ptsname");
  slave = open(name, O_RDWR);
  if (slave < 0) fail_io("open \"%s\"", name);
  ...
  */
  ...
  if (pipe(pipe_fds) < 0) fail_io("pipe");
  ...
  if (sigaction(SIGCHLD, &sa, NULL) < 0) fail_io("sigaction");
  ...
  if (pid < 0)
     ...
     */
    if (old_itimerval.it_value.tv_usec < 0 ||
        old_itimerval.it_value.tv_usec > 999999) {
      old_itimerval.it_value.tv_usec = 999999;
    }
    if (old_itimerval.it_interval.tv_usec < 0 ||
    ...
    if (dup2(slave, STDOUT_FILENO) < 0) fail_io("dup2");
