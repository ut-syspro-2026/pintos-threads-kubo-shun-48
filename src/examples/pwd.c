...
#define MAX_LEVEL 20
  ...
  if (cwd_size < 2) return false;

  /* Get inumber for current directory. */
  if (!get_inumber(".", &child_inum)) return false;
  ...
  for (;;) {
    ...
    /* Compose "../../../..", etc., in NAME. */
    if ((namep - name) > MAX_LEVEL * 3) return false;
    *namep++ = '.';
    *namep++ = '.';
    *namep = '\0';
    ...
    *namep++ = '/';
    ...
    if (parent_inum == child_inum) break;
    ...
    for (;;) {
      ...
      if (test_inum == child_inum) break;
    }
    close(parent_fd);

    /* Prepend "/name" to CWD. */
    if (!prepend(namep - 1, cwd, &cwd_len, cwd_size)) return false;
